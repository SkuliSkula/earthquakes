...
function loadEarthquakes() {
    d3.json("https://www.apis.is/earthquake/is", function(error, json) {
        if(error)
...
function initEartquakes(json) {
    ...
    rScale = d3.scale.linear()
                ...
                .range([1,10]);
    // Create a scale forthe depth
    var maxDepthArray = dataset.map(function(d){
       return d.depth; 
    });
    var colorScale = d3.scale.linear().domain([0,d3.max(maxDepthArray)]).range(["#ff7d4d","#b33000"]);
